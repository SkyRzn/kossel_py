...
class Screw:
	...
	def z_sup(self, h):
		res = cylinder(h + 0.1, d = self.d + self.gap_thread * 2 + 0.1, fn = 16)
		#res -= cylinder(h + 2, d = self.d + self.gap_thread, fn = 16) << [0, 0, -1]
		return res
	
	def contersum_hole(self, h1, h2, d2, h3 = 0):
		res = cylinder(h1 + 0.01, d = self.d + self.gap_thread * 2, fn = 32)
		res <<= [0, 0, -h1]
		res += cylinder(h2, d = self.d + self.gap_thread * 2, d2=d2, fn = 32)
		if h3:
			res += cylinder(h3, d = d2, fn = 32) << [0, 0, h2 - 0.01]
		return res
	
m3 = Screw(3.0, 5.5, 0.25, 0.33, 0.5)
